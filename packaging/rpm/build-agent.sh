#!/bin/sh
#
# Build rdhm-agent RPM package.
#
# Run from the project root directory:
#   ./packaging/rpm/build-agent.sh
#
# Prerequisites: rpm-build, cmake, gcc-c++
#
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
SPEC="$SCRIPT_DIR/rdhm-agent.spec"

VERSION=$(grep '^Version:' "$SPEC" | awk '{print $2}')
PKG_NAME="rdhm-agent-${VERSION}"
...
rpmbuild -bb \
    ...
    "$RPMBUILD_DIR/SPECS/rdhm-agent.spec"
